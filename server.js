...
wss.on('connection', function(ws) {
    ...
    ws.on('message', function(message) {
        ...
        // Added close message to stop incoming data
        if (message === 'close') {
            if (destroy) {
                destroy();
                destroy = null;
...
// Express serving the two variables at a route so client can obtain
app.get("/api/data", (req, res) => {
    res.json({
        numberItems: NUM_ITEMS,
        numberMessages: MESSAGES_PER_SECOND,
    })
})

// Express is serving the static react files after running 'build'
app.get("*", (req, res) => {
